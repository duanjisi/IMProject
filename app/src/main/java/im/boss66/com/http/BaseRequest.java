...
public abstract class BaseRequest<T> {
    ...
    protected BaseRequest(String tag, Object... params) {
        mGenericPojoClazz = (Class<T>) TypeResolver.resolveRawArgument(BaseRequest.class, getClass());
        mTag = tag;
        mParams = params;
    }
...
//                            jsonDao.save(switchJsonEntity(rspContent));
                            if (BaseRequest.this instanceof PageModel) {
                                retT = (T) JSON.parseObject(preRspPojo.result,
                                        ((PageModel) BaseRequest.this).getSubPojoType());
                            } else {
                                if (isParse()) {
...
//        MycsLog.d("Request\nAPI:" + getApiUrl() + "?" + OkHttpUtil.formatParams(encryptionParams));
        return getApiPath() + "?" + OkHttpUtil.formatParams(encryptionParams);
    }
